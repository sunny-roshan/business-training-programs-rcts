...
standardise_data <- function(data,
                             ...
                             sales_col,
                             profits_col,
                             baseline_sales_col = NULL,
                             baseline_profits_col = NULL) {
  summarization <- data %>%
  ...
  if (!is.null(baseline_sales_col) &&
      baseline_sales_col %in% colnames(data)) {
  ...
  if (!is.null(baseline_profits_col) &&
      baseline_profits_col %in% colnames(data)) {
    summarization <- summarization %>%
      mutate(
...
calculate_z_scores <- function(data,
  ...
  if (!is.null(baseline_sales_col) &&
      baseline_sales_col %in% colnames(data)) {
    data <- data %>%
      mutate(Baseline_Revenue_z = (!!sym(baseline_sales_col) - avgbaselinerevenue) / sdbaselinerevenue)
  }
  
  if (!is.null(baseline_profits_col) &&
      baseline_profits_col %in% colnames(data)) {
    data <- data %>%
      mutate(Baseline_Profit_z = (!!sym(baseline_profits_col) - avgbaselineprofits) / sdbaselineprofits)
  }
