...
Kenya_clean <- Kenya_clean %>% 
  mutate(Profit_zscore = (tprofits - avgprofits)/sdprofits,
         Revenue_zscore = (trevenue - avgrevenue)/sdrevenue) %>%
  relocate(c(avgprofits, sdprofits, Profit_zscore, trevenue, avgrevenue, sdrevenue, Revenue_zscore), .after = tprofits)  
...
Kenya_clean <- Kenya_clean %>% 
  mutate(treatment_indicator = as.numeric(treat2 == 3), fup_indicator = as.numeric(wave != 0))
...
Kenya_clean <- Kenya_clean %>%
  ...
  select(treatment_indicator, Profit_zscore, Revenue_zscore, Baseline_Profit_z, Baseline_Revenue_z) %>%
  mutate(urban = 1, female = 1, group = "Kenya")
