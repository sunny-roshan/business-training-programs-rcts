...
# Set wd and load profits csv dataset ----
getwd()
Profits <- read.csv(here("cleaned-datasets", "Profits summary stats.csv"))

# model 1 - uniform prior variance, only summary data, no covariates
defaultvarpiorlimit <- 10 * sd(Profits$tau_p)
defaultvarpiorlimit
...
Profits_BHM <- baggr(
  ...
  prior_hypersd = uniform(0, defaultvarpiorlimit),
  iter = 15000,
  chains = 8,
...
# Second prior: changing prior on on hypersd to half-Cauchy(0,25) - better than uniform (less weight on extreme hetero)
Profits_BHM_2 <- baggr(
  data = Profits,
  model = "rubin",
  pooling = "partial",
...
# Third prior: an even flatter half Cauchy(0,40)
Profits_BHM_3 <- baggr(
  data = Profits,
  model = "rubin",
  pooling = "partial",
  prior_hypermean = normal(0, 100),
  prior_hypersd = cauchy(0, 40),
  iter = 15000,
  chains = 8,
  control = list(adapt_delta = 0.95)
)
...
# the stan fit object contains the hypermean, hypervar, and shrunk tau parameters for each iteration; as well as the summary figures, which are what we want
...
tab_lab <- c("Hypermean (mu)", "Hyper-sd (tau)", Profits$group_p)
...
## Uniform prior
pdf(file = "Posterior intervals 1 ordered.pdf",
    width = 8,
    height = 4)
...
## Cauchy(0,25) prior
pdf(file = "Posterior intervals 2 ordered.pdf",
    width = 8,
    height = 4)
...
## Cauchy(0,40) prior
pdf(file = "Posterior intervals 3 ordered.pdf",
    width = 8,
    height = 4)
...
## Uniform prior
pdf(file = "Posterior distributions 1 ordered.pdf",
    width = 8,
    height = 6.5)
plot(Profits_BHM,
     style = "areas",
...
## Cauchy(0,25) prior
pdf(file = "Posterior distributions 2 ordered.pdf",
    width = 8,
    height = 6.5)
plot(Profits_BHM_2,
     style = "areas",
...
## Cauchy(0,40) prior
pdf(file = "Posterior distributions 3 ordered.pdf",
    width = 8,
    height = 6.5)
plot(Profits_BHM_3,
     style = "areas",
...
## Uniform
pdf(file = "Original and posteriors 1 forest.pdf",
    width = 8,
    height = 5)
...
##Cauchy(0,25)
pdf(file = "Original and posteriors 2 forest.pdf",
    width = 8,
    height = 5)
...
##Cauchy(0,40)
pdf(file = "Original and posteriors 3 forest.pdf",
    width = 8,
    height = 5)
...
## uniform
pdf(file = "Posterior hypermean treatment distribution 1.pdf",
    width = 6,
    height = 6)
...
## Cauchy(0,25)
pdf(file = "Posterior hypermean treatment distribution 2.pdf",
    width = 6,
    height = 6)
...
## Cauchy(0,40)
pdf(file = "Posterior hypermean treatment distribution 3.pdf",
    width = 6,
    height = 6)
...
BHM_compare <- baggr_compare(
  ...
  "Partial pooling BHM" = Profits_BHM_2,
  what = "pooling"
)
BHM_compare
summary(BHM_compare)
...
rownames(BHM_compare_2_df) <- c(
  ...
  "Cauchy (0,25): Mean",
  "Cauchy (0,40): Mean",
  "U[0,275]: Hyper-SD (tau)",
  "Cauchy (0,25): Hyper-SD (tau)",
  "Cauchy (0,40): Hyper-SD (tau)",
  "U[0,275]: Posterior Predictive Effect",
...
pdf(file = "Posterior hyper-effect distribution - Compare prior assumptions 2.pdf",
    width = 8,
    height = 6)
effect_plot(
  "U[0,275]" = Profits_BHM,
...
colnames(Profits_crossval_output) <- c("Left-out study",
                                       "Hypermean (mu)",
                                       "Hyper-SD (tau)",
                                       "LOO Info Criterion")
...
## With covariates ----
Profits_expanded <- read.csv(here("cleaned-datasets", "Profits stats with covariates.csv"))

Profits_femalescontrol <- baggr(
  Profits_expanded,
  outcome = "outcome",
  group = "group",
  pooling = "partial",
...
Profits_2controls <- baggr(
  ...
  prior_hypermean = normal(0, 50),
  prior_hypersd = cauchy(0, 20),
...
Profits_urbancontrol <- baggr(
  Profits_expanded_minus1,
  outcome = "outcome",
  group = "group",
  pooling = "partial",
  model = "rubin",
...
# change the rownames from mu, tau, tau[1], etc. to the study names
group_3control <- Profits_expanded$group_p[-7]
...
pdf(file = "Posterior hyper-effect distribution - controls or no.pdf",
    width = 8,
    height = 6)
effect_plot("Cauchy (0,25)" = Profits_BHM_2, "3 controls" = Profits_3controls) +
  coord_cartesian(xlim = c(-30, 100), ) + theme(legend.position = "top") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 20)) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme(text = element_text(family = "serif", size = 14))
...
colnames(Profits_3controls_crossval_output) <- c("Left-out study",
                                                 "Hypermean (mu)",
                                                 "Hyper-SD (tau)",
                                                 "LOO Info Criterion")
...
Profits_BHM_Data8 <- baggr(
  Profits_Data,
  model = "rubin",
  pooling = "partial",
  prior_hypermean = normal(0, 100),
  prior_hypersd = uniform(0, defaultvarpiorlimit_Data8),
  iter = 20000,
  chains = 8,
  control = list(adapt_delta = 0.95)
)
...
pdf(file = "Posterior intervals.pdf",
    width = 8,
    height = 4)
...
pdf(file = "Posterior intervals ordered.pdf",
    width = 8,
    height = 4)
...
# with the actual posterior distributions:
pdf(file = "Posterior distributions ordered.pdf",
    width = 8,
    height = 6)
plot(
  Profits_BHM_2_Data8,
...
pdf(file = "Original and posteriors.pdf",
    width = 8,
    height = 4)
...
pdf(file = "Posterior treatment distribution.pdf",
    width = 6,
    height = 6)
...
# Comparison: comparing Bayesian FE, no-pooling, BHM with default priors (= Profits_BHM) ----
# For Bayesian full pooling and no-pooling there is no prior variance specification.
...
BHM_compare_Data8 <- baggr_compare(
  "No pooling model" = nopoolcomparo_Data8,
  "Full pooling model" = fullpoolcomparo_Data8,
  "Partial pooling BHM" = Profits_BHM_2_Data8,
  what = "pooling"
)
BHM_compare_Data8
summary(BHM_compare_Data8)
BHM_compare_Data8_df <- rbind(
  BHM_compare_Data8$mean_trt,
  BHM_compare_Data8$sd_trt,
  BHM_compare_Data8$posteriorpd_trt
)

# drop the no-pooling since that means there are no aggregate estimates:
BHM_compare_Data8_df <- BHM_compare_Data8_df[c(2, 3, 5, 6, 8, 9), ]
BHM_compare_Data8_df
rownames(BHM_compare_Data8_df) <- c(
...
BHM_compare_Data8_2 <- baggr_compare("Default priors" = Profits_BHM_Data8,
                                     "Cauchy prior and debugging" = Profits_BHM_2_Data8)
summary(BHM_compare_Data8_2)

BHM_compare_Data8_2_df <- rbind(
  BHM_compare_Data8_2$mean_trt,
  BHM_compare_Data8_2$sd_trt,
  BHM_compare_Data8_2$posteriorpd_trt
)

rownames(BHM_compare_Data8_2_df) <- c(
  "Default Model: Mean",
  "Cauchy Prior: Mean",
  "Default Model: SD",
  "Cauchy Prior: SD",
...
pdf(file = "Treatment effect distribution - Compare prior assumptions.pdf",
    ...
    height = 6)
plot(
  baggr_compare("Default priors" = Profits_BHM_Data8, "Cauchy prior" = Profits_BHM_2_Data8)
) +
...
colnames(Profits_crossval_Data8_output) <- c("Left-out study",
                                             "Hypermean (tau)",
                                             "Hyper-SD",
                                             "LOO Info Criterion")
...
# comparing LOOIC/ELPD for full pooling vs BHM partial pooling - not extracted:
full_pool_crossval_Data8 <- loocv(
  data = Profits_Data,
  model = "rubin",
  pooling = "full",
  prior_hypermean = normal(0, 100)
